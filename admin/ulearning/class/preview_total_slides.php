<?php
class Preview_total_slides extends Simplexi_Controller{
	public function run($aArgs){
		...
		$limit = 1;
		$total = $this->db->ulearning_model->db_select('tb_ulearning_slide', array("funit_id"), array("funit_id" => $aArgs['unit_id']));
		$aData['preview'] = $this->db->ulearning_model->db_select('tb_ulearning_slide', null, array("funit_id" => $aArgs['unit_id']), array("ASC" => 4,"DESC" => 1), $limit, $aArgs['offset']);
			...
			$aData['offset'] = $aArgs['offset']+1;
			$aData['total'] = count($total);
			$aData['unit_id'] = $aArgs['unit_id'];
		...
		if($aData['preview'][0]['fexam_type_id'] == 1){
			...
			foreach($aQuestions as $key => $val){ 
				...
				if($val['fimage']){
					$str .= '<img style="float:left; margin:7px 7px 20px 15px" src="' . BASE_URL . 'image.php?w=250&path=ulearning/question_upload/'. $val['fimage'] .'" width="250"/>';
				}
			...
			foreach($aWords as $key => $val){
				...
				$str .= '<div style="width:auto;display:inline-block;">';
				$str .= '<p style="margin-top:10px"><img src="' . BASE_URL . 'image.php?w=300&path=ulearning/question_upload/'. $val['fimage'] .'" width="300" /></p>';
			...
			foreach($aSentence as $key => $val){
				...
				$str .= '<div class="m_video_box"><img src="' . BASE_URL . 'image.php?h=244&cr=16:9.49&path=ulearning/question_upload/'. $val['fimage'] .'" width="412" height="244" /></div>';
				if($val['fvoice'] == 1){
		...
		$this->display("ulearning/preview_total_slides",$aData,false);
	}
}
?>