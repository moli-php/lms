<?php
class Homework extends Simplexi_Controller{
	public function run($aArgs){
		$this->library("checkLogin");
		$this->model('ulearning_model');
		$this->importJS('ulearning/homework');
		$this->importJS('ulearning/preview_total_slides');
		$fbranch_idx = $_SESSION['idx'];
		$aData['fbranch_idx'] = $fbranch_idx;
		/////////////////////////////////////////////////////////////////
		if($fbranch_idx == 1)
			$this->db->ulearning_model->db_where(array('fdepth1' => 2));
		else
			$this->db->ulearning_model->db_where(array('fdepth1' => 2,'fbranch_idx' => $fbranch_idx));
		...
		if($fbranch_idx == 1)
			$this->db->ulearning_model->db_where(array('ftitle LIKE' => $aData['search'],'fdepth1' => 2));
		else
			$this->db->ulearning_model->db_where(array('ftitle LIKE' => $aData['search'],'fdepth1' => 2,'tb_ulearning_unit.fbranch_idx' => $fbranch_idx));
		$aData['iTotalRows'] = $this->db->ulearning_model->db_count('tb_ulearning_unit INNER JOIN tb_ulearning_category ON tb_ulearning_unit.fcategory_id = tb_ulearning_category.fcategory_id',array('fdepth1' => 2));
		
		if($fbranch_idx == 1)
			$this->db->ulearning_model->db_where(array('ftitle LIKE' => $aData['search'],'fdepth1' => 2));
		else
			$this->db->ulearning_model->db_where(array('ftitle LIKE' => $aData['search'],'fdepth1' => 2,'tb_ulearning_unit.fbranch_idx' => $fbranch_idx));
		$aData['homework'] = $this->db->ulearning_model->db_select('tb_ulearning_unit INNER JOIN tb_ulearning_category ON tb_ulearning_unit.fcategory_id = tb_ulearning_category.fcategory_id',array('funit_id','fcategory_name as course','ftitle','fdate','fdepth2'),null,array('ASC' => 1),$limit,$offset);
		if($aData['homework']){
			foreach($aData['homework'] as $key => $val):
				$aData['homework'][$key]['num'] = $key+1;
				$category2 = $this->db->ulearning_model->db_select('tb_ulearning_category',array('fcategory_name'),array('fcategory_id' => $val['fdepth2']));
				$aData['homework'][$key]['slideCount'] = $this->db->ulearning_model->db_count('tb_ulearning_slide',array('funit_id' => $val['funit_id']));
				$aData['homework'][$key]['category'] = $category2[0]['fcategory_name'];
			endforeach;
		}
		/////////////////////////////////////////////////////////////////
		
		$this->display("ulearning/homework",$aData);
	}
}
?>