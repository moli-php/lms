<?php
class Preview extends Simplexi_Controller{
	public function run($aArgs){
		$this->library("checkLogin");
		$this->model('ulearning_model');
		$this->importJS('ulearning/preview');
		...
		if($fbranch_idx == 1)
			...
			$this->db->ulearning_model->db_where(array('ftitle LIKE' => $aData['search'],'fdepth1' => 2,'tb_ulearning_unit.fbranch_idx' => $fbranch_idx));
		$aData['preview'] = $this->db->ulearning_model->db_select('tb_ulearning_unit INNER JOIN tb_ulearning_category ON tb_ulearning_unit.fcategory_id = tb_ulearning_category.fcategory_id',array('funit_id','fcategory_name as course','ftitle','fdate','fdepth2'),null,array('ASC' => 1),$limit,$offset);
		if($aData['preview']){
			foreach($aData['preview'] as $key => $val):
				$aData['preview'][$key]['num'] = $key+1;
				$category2 = $this->db->ulearning_model->db_select('tb_ulearning_category',array('fcategory_name'),array('fcategory_id' => $val['fdepth2']));
				$aData['preview'][$key]['slideCount'] = $this->db->ulearning_model->db_count('tb_ulearning_slide',array('funit_id' => $val['funit_id']));
				$aData['preview'][$key]['category'] = $category2[0]['fcategory_name'];
			endforeach;
		}
		/////////////////////////////////////////////////////////////////
		
		$this->display("ulearning/preview",$aData);
	}
}
?>