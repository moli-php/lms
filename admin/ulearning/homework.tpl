<?php echo Common::displayMenu(true,array("label"=>"Add Homework","most_action"=>"homework.open"));?>
<div>
	<div class="top_2">
		<div class="apply_action">
			...
			<a href="javascript:window.location.href = common.getBaseUrl()+'/admin/ulearning/?action=homework'" class="btn_apply">Total</a>
		</div>
	</div> <br /> <br />
		<div class="sidebar">
			<div class="category">
				...
				<ul>
					...
					foreach($depth1 as $key => $val):
						if(empty($val['fdepth2'])){
							echo "<li>
								<a href='javascript:homework_accordion($key)' class='link'><span class='plus$key'>+</span> ".$val['fcategory_name']."</a>
								<a href='javascript:void(0)' class='count'>(".$val['count'].")</a>
							</li>";
							foreach($depth1 as $k => $v):
								if(!empty($v['fdepth2']) && $v['fdepth2'] == $val['fcategory_id'])
									echo "<li class='sub_homework$key' style='display:none'>
		...
		<div id="content">
					...
					if($homework){
						foreach($homework as $key => $val):
							echo '<tr>
								...
								<td class="title"><a href="javascript:homework.modify('.$val['funit_id'].',\''.$val['ftitle'].'\')">'.$val['ftitle'].'</a></td>
							...
							echo '" class="btn_go" title="Total Slide View Page">V</a>
									<a href="javascript:homework.move('.$val['funit_id'].')" class="btn_go" title="Manage Slide">M</a>
									<a href="javascript:homework.deleteOpenPopUp('.$val['funit_id'].')" class="btn_go" title="Delete">D</a>
								</td>
			...
			<div class="popup_window deleteHomeworkPopUp" style="display:none;">
				...
				<div class="action_btn">
					<a href="javascript:void(0)" class="btn_save fr" title="Delete Data" onclick="homework.remove()">Delete</a>
					<a href="javascript:void(0)" class="btn_save" title="Cancel" onclick="homework.deleteClosePopUp()">Cancel</a>
				</div>
			</div>
			<div class="popup_window add_homework" style="display:none;">
				<form name = "addHomework">
				<table border="0" cellspacing="0" cellpadding="0" class="table_popup">
					<col width="75px" />
					<col width="200px" />
					<!--<tr><th colspan="2" class="popup_title">Add Homework &raquo;</th></tr>
					<tr><td colspan="2"></td></tr>-->
					<tr>
						<th>1 depth<span class="neccesary">*</span></th>
						<td>
							<select class="depth depth1" name="depth1"  onchange="homework.depth1(this)" validate="required">
								<option></option>
								<?php
									foreach($depth1 as $key => $val):
										if(empty($val['fdepth2'])){
					...
					<tr>
						...
						<td><input type="text" name="title" maxlength="80" value="" class="fix2 homework_title" validate="required"/></td>
					</tr>
				</table>
				<div class="action_btn">
					<a href="javascript:homework.save()" class="btn_save fr" title="Save changes">Submit</a>
					<a href="javascript:homework.close();" class="btn_save" title="Cancel">Cancel</a>
				</div>
				
			</div>
			<div class="popup_window modify_homework" style="display:none;">
				<form name = "modifyTitle">
					<table border="0" cellspacing="0" cellpadding="0" class="table_popup">
						<col width="250px" />
						<!--<tr><th colspan="2" class="popup_title">Modify Title &raquo;</th></tr>-->
						<tr>
							<th colspan="2"><input type="text" name="title_modify" maxlength="100" class="fix3 homework_title_modify" validate="required" /><input type="hidden" class=" homework_id_modify" /> </th>
						</tr>
					</table>
				</form>	
					<div class="action_btn">
						<a href="javascript:homework.modifySave()" class="btn_save fr" title="Save changes">Submit</a>
						<a href="javascript:homework.modifyClose()" class="btn_save" title="Cancel">Cancel</a>
