<?php
class classCancelled extends Simplexi_Controller
{
	public function run($aArgs)
	{
	    ...
	    if(isset($_SESSION['idx'])){
	        $this->classCancelledReady($aArgs);
        } else {
            echo "<script>location.href='" . BASE_URL . "admin/'</script>";
        }
	}

	private function classCancelledReady($aArgs)
	{
	    ...
	    $aData['cancelledInfo'] = $this->query($sQueryAll.$sLimit);
	    ...
	    if($aData['iTotalRow'] >=1) {
	        foreach($aData['cancelledInfo'] as $key=>$val) {
	            $aData['cancelledInfo'][$key]['iCount'] = $aData['iTotalRow'] - $iOffset - $i;
	            ++$i;
	        }
	    }

		$this->display("class/tpl/classCancelled", $aData);
	}
