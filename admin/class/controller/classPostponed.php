<?php
class classPostponed extends Simplexi_Controller
{
	public function run($aArgs)
	{
	    ...
	    if(isset($_SESSION['idx'])){
	        $this->classPostponedReady($aArgs);
        } else {
            echo "<script>location.href='" . BASE_URL . "admin/'</script>";
        }
	}

	private function classPostponedReady($aArgs)
    {
	    ...
	    $aData['posponedInfo'] = $this->query($sQueryAll.$sLimit);
	    ...
	    if($aData['iTotalRow'] >=1) {
	        foreach($aData['posponedInfo'] as $key=>$val) {
	            $aData['posponedInfo'][$key]['iCount'] = $aData['iTotalRow'] - $iOffset - $i;
	            ++$i;
	        }
	    }

		$this->display("class/tpl/classPostponed", $aData);
	}
