<?php echo Common::displayMenu(true,array("label"=>"Add New Banner","most_action"=>"settings.addBannerUrl"));?>

		<div class="top_2">
			<ul class="sort_view">
				<li class="active all"><a href="<?php echo BASE_URL.'admin/event';?>">All (<?php echo $aData[0]['iCount']['all']; ?>)</a></li>
				...
				<li style="float:right;background:none"><input type="text" /><input type="submit" value="search" id="banner_search" class="btn_apply" onclick="javascript: if($(this).prev().val())window.location.href='<?php Common::urlInclude('search');?>'+ $(this).prev().val();" /></li>
			</ul>
			<div class="show_rows">
				Show Rows
		...
		<table cellspacing="0" class="post_table banner" id="banner_table_list">
			<colgroup>
				<col width="40px" />
				<col width="40px" />
				<col width="80px" />
				...
				if(count($aData[0]['aData']['getData']) > 0){
					...
					?>
						<tr>
							...
							<td class="preview"><img src="<?php echo BASE_URL;?>image.php?w=200&path=/event/banner/<?php echo $val['banner_image'];?>" /></td>
							<td><a href="<?php echo BASE_URL;?>admin/event/?action=add_banner&idx=<?php echo $val['idx'].$aData[0]['url_args']; ?>"><?php echo $val['title'];?></a></td>
							<td><?php echo date('M d, Y g:i:s a',$val['date_created']);?></td>
							<td class=""><a href="<?php echo BASE_URL;?>admin/event/?action=add_banner&idx=<?php echo $val['idx'].$aData[0]['url_args']; ?>"><img src="<?php echo BASE_URL;?>images/wrench-screwdriver.png" alt="Management" /></a>&nbsp;<a href="#" class="delete_record" id="<?php echo $val['idx']."|banner"; ?>"><img src="<?php echo BASE_URL;?>images/icons/cross-circle.png" alt="Delete" /></a></td>
						</tr>
					<?php 
					}
				}else{?>
		...
		<div class="bottom">
			<div class="apply_action">
				<a href="#" id="delete_records" rel="banner" class="btn_apply">Delete</a>
			</div>
		</div>
