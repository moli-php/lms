<?php echo Common::displayMenu(true,array("label"=>"Add New Template","most_action"=>"EmailTpl"));?>
<!-- hidden -->
...
<input type="hidden" id="orderset" value="<?php echo isset($aArgs['order']) ?  $aArgs['order']  : "asc";?>" />
		...
		<thead>
			<tr>
				...
				<th>No.</th>
				<th><a href="#" class="sort_down" id="title">Title</a></th>
		...
		<?php if($listcount>=1){?>
		 <?php foreach ($aData as $list): ?>
			<tr>
				...
				<td><a href="javascript:EmailTpl('modify','<?php echo $list['idx']?>')"><?php echo $list['tpl_title']?></a></td>
		...
		$pagination = 'false';
		echo "<tr><td colspan='4'>Empty List!</td></tr>";
		}
