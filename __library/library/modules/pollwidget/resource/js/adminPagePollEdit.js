...
var adminPagePollEdit = {
        CheckForm: function(){
            if(!oValidator.formName.getMessage('pollwidget_edit'))
                ...
                $('#pollwidget_edit').submit();
                //this.check_overlap(s_date, e_date);

                //$('#poll_new').submit(e_date);
                }
        },
        ...
        add_choice : function(sel)
        {
            ...
            $('ul#content_choice_list_'+qno).append('<li style="padding: 2px;"><input type="text" name="' + qno + '_choice[]" class="input_table" style="width:500px;" />\n<span class="btn_input"><input type="button" id="btn_cont_plus_'+qno+'" class="btn_cont_plus" alt="add choice" onclick="javascript: adminPagePollEdit.add_choice(this);" style="width: 20px;" value="+"/>\n<input type="button" id="btn_cont_minus_'+qno+'" class="btn_cont_minus" onclick="javascript: adminPagePollEdit.remove_choice(this);" alt="delete choice" style="width: 20px;" value="-"/>\n<input type="button" id="btn_cont_top_'+qno+'" class="btn_cont_top" alt="move choice top" onclick="javascript: adminPagePollEdit.move_choice(this);" style="width: 20px;" value="<" />\n<input type="button" id="btn_cont_bt_'+qno+'" class="btn_cont_bt" alt="move choice bottom" onclick="javascript: adminPagePollEdit.move_choice(this);" style="width: 20px;" value=">" />\n</span></li>');
        ...
        {
            var qno = $(sel).parents('table').index()+1;
            $('ul#content_choice_list_'+qno).append('<li style="padding: 2px;"><input type="text" name="'+qno+'_choice[]" class="input_table" style="width:500px;" />\n<span class="btn_input"> <input type="button" id="btn_cont_plus_'+qno+'" class="btn_cont_plus" alt="" onclick="adminPagePollEdit.add_choice(this)"  style="width: 20px;" value="+"/>\n<input type="button" id="btn_cont_minus_'+qno+'" class="btn_cont_minus" alt="" onclick="adminPagePollEdit.remove_choice(this)" style="width: 20px;" value="-"/>\n<input type="button" id="btn_cont_top_'+qno+'" class="btn_cont_top" alt="" onclick="adminPagePollEdit.move_choice(this)" style="width: 20px;display:none" value="<"/>\n<input type="button" id="btn_cont_bt_'+qno+'" class="btn_cont_bt" alt="" onclick="adminPagePollEdit.move_choice(this)" style="width: 20px;display:none" value=">"/></span>\n</li>');
        ...
        remove_choice : function(sel)
        {
            ...
            if(li_siz == 0) {
                $('#choice_txt_explan_'+qno).html('<span class="btn01"><input type="button" id="btn_add_a_choice_'+qno+'" value="&nbsp;+ add a choice&nbsp;" style="font-size:11px; width: 120px;" onclick="adminPagePollEdit.add_choice_noarr(this)"/></span>');
            } else if(li_siz == 1) {
        ...
        add_opinion_popup : function (sel) 
        {
            ...
            if(chcount == 0) {
                ...
                $("#content_choice_list_"+qno+" > li").each(function(index, value) {

                    var choice = $.trim($(".input_table", this).val());
                    var chk = $(this).attr('comment') == '1' ? 'checked' : '';

                    if(choice.length != 0) {
                        if($('#setting_opinion_'+qno+'_'+index).val() == 1)
                            var check = 'checked';
                        else
                            var check = '';
                        
                        $('.op').append('<tr class="op_added"><td class="fir subject">'+ choice +'</td><th><input type="checkbox" class="chk_box" name="op_check[]" '+ chk +' value="" '+check+' style="width:20px;"/></th></tr>');
                    } else {
        ...
        add_question_form : function()
        {
            ...
            tbl += '<tr id="disp_num_tr_'+curr_num+'"><th>Question number</th><td><span id="span_num_'+curr_num+'">'+curr_num+'<input type="hidden" name="qnum[]" value="'+curr_num+'" /></span><div style="float:right;"  id="quest_btn_'+curr_num+'"><span class="btn01 ml10"><input type="button" value="&nbsp;Additional option&nbsp;" class="btn_add_opinion" id="btn_add_opinion_'+curr_num+'" onclick="adminPagePollEdit.add_opinion_popup(this)" style="width: 110px;"/></span>\n<span class="btn01 ml10" ><input type="button" value="&nbsp;- delete&nbsp;" class="btn_delete_a_question" id="btn_delete_a_question_'+curr_num+'" onclick="adminPagePollEdit.delete_a_question(this)" style="width: 110px;"/></span></span></div></td></tr>';
            ...
            tbl += '<td><input type="radio" id="choice_one_answer_'+curr_num+'" name="question_choice_type_'+curr_num+'" class="radio_type" checked="checked" value="0" onclick="adminPagePollEdit.choice_option_on(this)" style="width: 12px; padding: 1px 5px 1px 15px; border: none;"/><label for="choice_one_answer_'+curr_num+'">One Answer</label>';
            tbl += '<input type="radio" id="choice_multiple_answers_'+curr_num+'" name="question_choice_type_'+curr_num+'" class="radio_type list_gab" value="1" onclick="adminPagePollEdit.choice_option_on(this)" style="width: 12px; padding: 1px 5px 1px 15px; border: none;"/><label for="choice_multiple_answers_'+curr_num+'">Multiple Answers</label>';
            tbl += '<input type="radio" id="choice_ranking_'+curr_num+'" name="question_choice_type_'+curr_num+'" class="radio_type list_gab" value="2" onclick="adminPagePollEdit.choice_option_off(this)" style="width: 12px; padding: 1px 5px 1px 15px; border: none;"/><label for="choice_ranking_'+curr_num+'">Ranking</label>';
            tbl += '<input type="radio" id="choice_description_'+curr_num+'" name="question_choice_type_'+curr_num+'" class="radio_type list_gab" value="3" onclick="adminPagePollEdit.choice_option_desc(this)" style="width: 12px; padding: 1px 5px 1px 15px; border: none;"/><label for="choice_description_'+curr_num+'">A Form of Description</label></td></tr>';
            ...
            tbl += '<input type="button" id="btn_cont_plus_'+curr_num+'" class="btn_cont_plus" alt="add choice" onclick="javascript: adminPagePollEdit.add_choice(this);" style="width: 20px;" value="+"/>\n';
            tbl += '<input type="button" id="btn_cont_minus_'+curr_num+'" class="btn_cont_minus" onclick="javascript: adminPagePollEdit.remove_choice(this);" alt="delete choice" style="width: 20px;" value="-"/>\n';
            tbl += '<input type="button" id="btn_cont_top_'+curr_num+'" class="btn_cont_top" alt="move choice top" onclick="javascript: adminPagePollEdit.move_choice(this);" style="width: 20px;display:none" value="<" />\n';
            tbl += '<input type="button" id="btn_cont_bt_'+curr_num+'" class="btn_cont_bt " alt="move choice bottom" onclick="javascript: adminPagePollEdit.move_choice(this);" style="width: 20px;display:none" value=">" />\n';
            tbl += '</span>';
